...
const getCardBorder = (card: QueueCard): string => {
  ...
  if (isCurrentSlot(reservation.startTime, reservation.endTime))
    return 'border-accent shadow-md ring-1 ring-accent/30'
  return 'border-gray-200 bg-white'
}

// ─── Cache helpers (simple in-memory to avoid re-fetching same doc) ─────────
    ...
    <div className="min-h-full p-4 md:p-6">
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 text-sm font-semibold text-primary shadow-sm transition-all hover:bg-accent/90 active:scale-95"
          >
      ...
      {isLoading ? (
        ...
        >
          ...
          <p className="font-semibold text-gray-500">Aucune réservation aujourd'hui</p>
        ...
        <div className="space-y-3">
          {cards.map((card, idx) => {
            ...
            const isCurrent = isCurrentSlot(res.startTime, res.endTime)
            ...
            return (
              <motion.div
                ...
                className={`relative overflow-hidden rounded-2xl border p-4 transition-shadow ${getCardBorder(card)}`}
              >
                {/* Current indicator */}
